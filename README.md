...
The core set of plugins bundles all dependencies that each plugin needs, in the plugin itself.
This is to avoid complexities with importmap until that matures a bit and prioritize simplicity.

You do still need an import map somewhere on the page, which tells plugins where to find picodash:
